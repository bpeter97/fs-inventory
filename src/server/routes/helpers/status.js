...
exports.patchStatus = (req, res) => {
    ...
    // Check to see if status already exist.
    Status.findOne({ label: update.label }).then((status) => {
        if (status) {
            // Return error if it already exists.
            errors.label = "A status with that label already exists";
            return res.status(400).json(errors);
        } else {
            Status.findOne({ initials: update.initials }).then((initials) => {
                if (initials) {
                    // Return error if it already exists.
                    errors.initials = "A status with the initials already exists";
                    return res.status(400).json(errors);
                    ...
                    // Save the new Status to the DB, lastly return the Status.
                    Status.findByIdAndUpdate(req.params.id, update, { new: true })
                        .then((status) => {
                            if(!status) {
                                return res.json({ error: "There was no status found" });
                            } else {
                                // Save a new notification
                                let note_data = {
                                    description: `A new status (${status.initials}) has been updated.`,
                                    users_read: []
                                };
...
exports.deleteStatus = (req, res) => {
    ...
    Status.findByIdAndRemove(req.params.id)
        .then((status) => {
            ...
            // Save a new notification
            let note_data = {
                description: `A new status (${status.initials}) has been deleted.`,
                users_read: []
            };
